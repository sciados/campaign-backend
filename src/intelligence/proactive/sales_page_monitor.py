...
class ProactiveSalesPageMonitor:
    ...
    async def queue_url_for_proactive_analysis(
        ...
        try:
            ...
            result = await self.db.execute(existing_check, {"url": url})
            if result.fetchone():
            ...
            insert_query = text("""
                ...
                VALUES (:url, :priority, :source, :metadata, 'pending', NOW())
                ON CONFLICT (url) DO UPDATE SET
                    priority = GREATEST(proactive_analysis_queue.priority, :priority),
                    updated_at = NOW()
            """)
    ...
    async def process_proactive_analysis_queue(self, batch_size: int = 3) -> Dict[str, Any]:
        """Process queued URLs for proactive analysis"""
        try:
            ...
            result = await self.db.execute(queue_query, {"batch_size": batch_size})
            pending_urls = [row[0] for row in result.fetchall()]
            
            if not pending_urls:
                return {"processed": 0, "status": "no_pending_urls"}
            ...
            return {
                "processed": processed,
                "failed": failed,
                "total_attempted": len(pending_urls),
                "status": "completed"
            }
            
        except Exception as e:
            logger.error(f"Error processing proactive analysis queue: {str(e)}")
    ...
    async def _perform_proactive_analysis(self, url: str) -> bool:
        ...
        try:
            # Use intelligence_crud to create analysis with new schema
            analysis_data = {
                "source_url": url,
                "analysis_method": "proactive_monitoring",
                "metadata": {"proactive": True, "automated": True}
            }
            
            # Create intelligence using the new CRUD system
    ...
    async def _update_queue_status(self, url: str, status: str):
        ...
        try:
            ...
            await self.db.execute(update_query, {"url": url, "status": status})
            await self.db.commit()
    ...
    async def _get_or_create_system_user(self):
        ...
        try:
            ...
            result = await self.db.execute(query, {"email": "system@proactive.internal"})
