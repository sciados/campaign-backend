        ...
        try:
            ...
            if result:
                ...
                return {
                    ...
                    "benchmarked_at": datetime.datetime.now()
                }
        ...
        for provider, health_data in self.health_cache.items():
            if "health" in health_data:
                ...
                analytics["provider_health"][provider] = {
                    ...
                    "last_checked": health.get("checked_at", datetime.datetime.now()).isoformat() if isinstance(health.get("checked_at"), datetime) else str(health.get("checked_at")),
            ...
            if pricing_data.get("status") == "success":
                ...
                if pricing:
                    ...
                    analytics["provider_costs"][provider] = {
                        ...
                        "last_updated": pricing_data.get("updated_at", datetime.datetime.now()).isoformat() if isinstance(pricing_data.get("updated_at"), datetime) else str(pricing_data.get("updated_at"))
                    }
        ...
        for content_type, rec_data in self.recommendations_cache.items():
            if "recommendation" in rec_data:
                ...
                analytics["optimization_recommendations"][content_type] = {
                    ...
                    "cost_savings": rec.cost_savings,
                    "quality_impact": rec.quality_impact,
                    "calculated_at": rec_data.get("calculated_at", datetime.datetime.now()).isoformat() if isinstance(rec_data.get("calculated_at"), datetime) else str(rec_data.get("calculated_at"))
                }
