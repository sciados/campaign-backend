...
class ScientificAuthorityEnhancer:
    ...
    def _get_best_provider(self) -> Optional[Dict]:
        """Get the best available AI provider for authority analysis"""
        
        # Prefer Anthropic Claude for scientific authority content
        for provider in self.ai_providers:
            if provider.get("name") == "anthropic" and provider.get("available"):
                logger.info("👨‍🔬 Using Anthropic Claude for scientific authority enhancement")
                return provider
        
        # Fallback to OpenAI
        for provider in self.ai_providers:
            if provider.get("name") == "openai" and provider.get("available"):
                logger.info("👨‍🔬 Using OpenAI for scientific authority enhancement")
                return provider
        
        logger.warning("⚠️ No AI providers available for scientific authority enhancement")
        return None
