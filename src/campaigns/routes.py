...
🔧 CRITICAL FIX: Background task async session management - COMPLETE FILE
"""
...
from src.core.database import get_async_db, AsyncSessionLocal  # 🔧 CRITICAL FIX: Import AsyncSessionLocal
...
# ============================================================================
# 🔧 CRITICAL FIX: BACKGROUND TASK FOR AUTO-ANALYSIS - COMPLETELY REWRITTEN
# ============================================================================
...
async def trigger_auto_analysis_task_fixed(
    ...
    try:
        ...
        # This is the key fix - we create our own session instead of relying on dependency injection
        async with AsyncSessionLocal() as db:
            try:
                ...
                # Get campaign with proper UUID handling
                campaign_result = await db.execute(
                    select(Campaign).where(
                        and_(Campaign.id == UUID(campaign_id), Campaign.company_id == UUID(company_id))
                    )
                )
                ...
                try:
                    ...
                    if analysis_result.get("intelligence_id"):
                        ...
                        # Create analysis summary using result data
                        analysis_summary = {
...
# ============================================================================
# 🎯 FIXED: Demo creation endpoint (this was corrupted in your file)
# ============================================================================
...
async def create_demo_with_live_analysis(
    ...
    """
    🎓 EDUCATIONAL: Create demo campaign with real-time analysis
    """
    try:
        ...
        # 🔧 CRITICAL FIX: Use the fixed background task
...
async def get_campaigns(
    ...
    try:
        ...
        try:
            ...
            user_demo_preference = {"show_demo_campaigns": True}
...
# ============================================================================
# CAMPAIGN CREATION (keeping existing logic with CRITICAL FIX)
# ============================================================================
...
async def save_progress(
    ...
    try:
        ...
        # 🔧 FIXED: Create timezone-aware datetime once
    ...
    try:
        ...
        if is_demo_campaign(campaign):
            ...
            if real_campaigns_count > 0:
                # ✅ User has real campaigns - allow deletion
